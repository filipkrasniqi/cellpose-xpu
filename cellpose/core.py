...
class UnetModel():
    ...
    def eval(self,
        ...
        if cell_threshold is None or boundary_threshold is None:
            try:
                thresholds = np.load(model_path +
                                     '_cell_boundary_threshold.npy')
                cell_threshold, boundary_threshold = thresholds
                core_logger.info(
                    '>>>> found saved thresholds from validation set')
            except:
                core_logger.warning(
        ...
        if not do_3D:
            for i in iterator:
                ...
                y, style = self._run_nets(img,
                                          net_avg=net_avg,
                                          augment=augment,
                                          tile=tile)
                if compute_masks:
                    maski = utils.get_masks_unet(y, cell_threshold,
                                                 boundary_threshold)
            ...
            for i in iterator:
                ...
                yf, style = self._run_3D(x[i],
                                         ...
                                         tile=tile)
                yf = yf.mean(axis=0)
                core_logger.info('probabilities computed %2.2fs' %
                                 (time.time() - tic))
                if compute_masks:
                    maski = utils.get_masks_unet(yf.transpose(
                        (1, 2, 3, 0)), cell_threshold, boundary_threshold)
                    maski = utils.fill_holes_and_remove_small_masks(
                        maski, min_size=min_size)
                else:
                    maski = None
    ...
    def _run_tiled(self,
        ...
        if imgi.ndim == 4:
            ...
            yf = np.zeros((Lz, self.nclasses, imgi.shape[-2], imgi.shape[-1]),
                          np.float32)
            styles = []
            if ny * nx > batch_size:
                ziterator = trange(Lz, file=tqdm_out)
                for i in ziterator:
                    yfi, stylei = self._run_tiled(imgi[i],
                                                  augment=augment,
                                                  bsize=bsize,
                                                  tile_overlap=tile_overlap)
                    yf[i] = yfi
            ...
            for k in range(niter):
                irange = slice(batch_size * k,
                               min(IMG.shape[0], batch_size * k + batch_size))
    ...
    def _run_3D(self,
        ...
        if anisotropy is not None:
            rescaling = [[rsz, rsz], [rsz * anisotropy, rsz],
                         [rsz * anisotropy, rsz]]
        else:
            rescaling = [rsz] * 3
    ...
    def train(self,
        ...
        train_data, train_labels, test_data, test_labels, run_test = transforms.reshape_train_test(
            train_data, train_labels, test_data, test_labels, channels,
            normalize)
        train_labels = [
            fastremap.renumber(label, in_place=True)[0]
        ...
        if self.nclasses == 3:
            ...
            train_classes = [
                np.stack((label, label > 0, utils.distance_to_boundary(label)),
        ...
        model_path = self._train_net(train_data,
                                     train_classes,
                                     test_data,
                                     test_classes,
                                     save_path=save_path,
        ...
        cell_threshold, boundary_threshold = self.threshold_validation(
            val_data, val_labels)
        np.save(model_path + '_cell_boundary_threshold.npy',
        ...
        for j, cell_threshold in enumerate(cell_thresholds):
            for k, boundary_threshold in enumerate(boundary_thresholds):
                ...
                for data in val_data:
                    output, style = self._run_net(data.transpose(1, 2, 0),
                                                  augment=False)
                    masks.append(
                        utils.get_masks_unet(output, cell_threshold,
                                             boundary_threshold))
            ...
            if j % 4 == 0:
                core_logger.info(
                    'best threshold at cell_threshold = {} => boundary_threshold = {}, ap @ 0.5 = {}'
                    .format(cell_threshold, boundary_thresholds[kbest],
                            aps[j, kbest, 0]))
        if self.nclasses == 3:
            jbest, kbest = np.unravel_index(
                aps.mean(axis=-1).argmax(), aps.shape[:2])
        else:
            jbest = aps.squeeze().mean(axis=-1).argmax()
            kbest = 0
        cell_threshold, boundary_threshold = cell_thresholds[
            jbest], boundary_thresholds[kbest]
        core_logger.info(
            '>>>> best overall thresholds: (cell_threshold = {}, boundary_threshold = {}); ap @ 0.5 = {}'
            .format(cell_threshold, boundary_threshold, aps[jbest, kbest, 0]))
    ...
    def _train_net(self,
        ...
        if isinstance(learning_rate, (list, np.ndarray)):
            ...
            if SGD:
                ...
                if self.n_epochs > 250:
                    LR = np.append(
                        LR,
                        self.learning_rate_const * np.ones(self.n_epochs - 100))
                    for i in range(10):
        ...
        diam_train = np.array([
            utils.diameters(train_labels[k][0])[0]
        ...
        if rescale:
            ...
            scale_range = 0.5
            core_logger.info('>>>> median diameter set to = %d' %
                             self.diam_mean)
        else:
            scale_range = 1.0
        ...
        self.net.diam_labels.data = torch.ones(
            1, device=self.device) * diam_train_mean

        nchan = train_data[0].shape[0]
        core_logger.info('>>>> training network with %d channel input <<<<' %
                         nchan)
        core_logger.info(
        ...
        for iepoch in range(self.n_epochs):
            ...
            rperm = inds_all[iepoch * nimg_per_epoch:(iepoch + 1) *
                             nimg_per_epoch]
            for ibatch in range(0, nimg_per_epoch, batch_size):
                inds = rperm[ibatch:ibatch + batch_size]
                rsc = diam_train[inds] / self.diam_mean if rescale else np.ones(
                    len(inds), np.float32)
                ...
                if self.unet and lbl.shape[1] > 1 and rescale:
                    lbl[:,
                        1] *= scale[:, np.newaxis, np.
                                    newaxis]**2  #diam_batch[:,np.newaxis,np.newaxis]**2
            ...
            if iepoch % 10 == 0 or iepoch == 5:
                ...
                if test_data is not None:
                    ...
                    for ibatch in range(0, len(test_data), batch_size):
                        inds = rperm[ibatch:ibatch + batch_size]
                        rsc = diam_test[
                            inds] / self.diam_mean if rescale else np.ones(
                                len(inds), np.float32)
                        imgi, lbl, scale = transforms.random_rotate_and_resize(
            ...
            if save_path is not None:
                if iepoch == self.n_epochs - 1 or iepoch % save_every == 1:
                    ...
                    ksave += 1
                    core_logger.info(
                        f'saving network parameters to {file_name}')
