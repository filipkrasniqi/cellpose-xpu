...
MODEL_DIR = pathlib.Path(
    _MODEL_DIR_ENV) if _MODEL_DIR_ENV else _MODEL_DIR_DEFAULT

MODEL_NAMES = [
    'cyto', 'nuclei', 'tissuenet', 'livecell', 'cyto2', 'general', 'CP', 'CPx',
...
class Cellpose():
    ...
    def __init__(
        ...
        self.sz = SizeModel(device=self.device,
    ...
    def eval(self,
        ...
        channels = [
            0, 0
        ] if channels is None else channels  # why not just make this a default in the function header?
...
class CellposeModel(UnetModel):
    ...
    def __init__(
        ...
        if model_type is not None or (pretrained_model and
                                      not os.path.exists(pretrained_model[0])):
            ...
            pretrained_model = [
                model_path(pretrained_model_string, j, self.torch)
    ...
    def eval(self,
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            ...
            if not model_loaded and (isinstance(self.pretrained_model, list) and
                                     not net_avg and not loop_run):
    ...
    def _run_cp(
        ...
        if compute_masks:
            ...
            if do_3D:
                ...
                for i in iterator:
                    outputs = dynamics.resize_and_compute_masks(
                        ...
                        interp=interp,
                        resize=resize,
                        min_size=min_size if stitch_threshold == 0 or nimg == 1
                        else -1,  # turn off for 3D stitching
                        use_gpu=self.gpu,
                        device=self.device)
            ...
            masks, dP, cellprob, p = masks.squeeze(), dP.squeeze(
            ), cellprob.squeeze(), p.squeeze()

        else:
            masks, p = np.zeros(0), np.zeros(
                0)  #pass back zeros if not compute_masks
    ...
    def train(self,
        ...
        if channels is None:
            models_logger.warning(
                'channels is set to None, input must therefore have nchan channels (default is 2)'
            )
        model_path = self._train_net(train_data,
                                     train_flows,
                                     test_data=test_data,
                                     test_labels=test_flows,
                                     save_path=save_path,
...
class SizeModel():
    ...
    def __init__(self, cp_model, device=None, pretrained_size=None, **kwargs):
        ...
        if pretrained_size is not None:
            self.params = np.load(self.pretrained_size,
    ...
    def train(
        ...
        if run_test:
            diam_test = np.array(
                [utils.diameters(lbl)[0] for lbl in test_labels])
        ...
        for iepoch in range(n_epochs):
            ...
            for ibatch in range(0, nimg, batch_size):
                ...
                diams[inds + nimg * iepoch] = np.log(diam_train[inds]) - np.log(
                    self.diam_mean) + np.log(scale)
            del feat
            if (iepoch + 1) % 2 == 0:
                models_logger.info('ran %d epochs in %0.3f sec' %
        ...
        A = np.linalg.solve(X @ X.T + l2_regularization * np.eye(X.shape[0]),
                            X @ y)
        ypred = A @ X
        models_logger.info('train correlation: %0.4f' %
                           np.corrcoef(y, ypred)[0, 1])

        if run_test:
            nimg_test = len(test_data)
            styles_test = np.zeros((nimg_test, 256), np.float32)
            for i in range(nimg_test):
                styles_test[i] = self.cp._run_net(test_data[i].transpose(
                    (1, 2, 0)))[1]
            diam_test_pred = np.exp(A @ (styles_test - smean).T +
                                    np.log(self.diam_mean) + ymean)
            diam_test_pred = np.maximum(5., diam_test_pred)
            models_logger.info('test correlation: %0.4f' %
