    ...
    def __init__(self, in_channels, out_channels, sz):
        ...
        for t in range(4):
            if t == 0:
                ...
                self.conv.add_module('conv_%d' % t,
                                     batchconv(out_channels, out_channels, sz))
        ...
        for n in range(len(nbase) - 1):
            if residual_on:
                self.down.add_module('res_down_%d' % n,
                                     resdown(nbase[n], nbase[n + 1], sz))
            else:
                self.down.add_module('conv_down_%d' % n,
                                     convdown(nbase[n], nbase[n + 1], sz))
    ...
    def __init__(self, nbase, sz, residual_on=True, concatenation=False):
        ...
        for n in range(1, len(nbase)):
            if residual_on:
                ...
                self.up.add_module(
                    'conv_up_%d' % (n - 1),
                    convup(nbase[n], nbase[n - 1], nbase[-1], sz,
                           concatenation))
...
class CPnet(nn.Module):
    ...
    def load_model(self, filename, device=None):
        ...
        if (device is not None) and (device.type != 'cpu'):
            ...
            self.__init__(self.nbase, self.nout, self.sz, self.residual_on,
                          self.style_on, self.concatenation, self.mkldnn,
                          self.diam_mean)
